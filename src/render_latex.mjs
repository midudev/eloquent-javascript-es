...
const { tokens } = transformTokens(markdown.parse(fs.readFileSync(file, 'utf8'), {}), {
...
let linkedChapter = null; let raw = false; let quote = false
...
const renderer = {
  ...
  code_inline (token) {
    if (noStarch) { return `\\texttt{${escape(token.content)}}` } else { return `\\lstinline\`${miniEscape(token.content)}\`` }
  },
...
contenido = contenido.replace(/\\lstinline`([^`]+)`/g, (_, code) => `\\texttt{${escapeLatex(code)}}`);
  ...
  meta_figure (token, _i, _t, newlines) {
    ...
    const graphics = `\\includegraphics[width=${width || '10cm'}]{${url}}`
...
\\epigraph{\\centering{\`\``
  ...
  meta_quote_close (token) {
    quote = false
    const { author, title, chapter, image } = token.args[0] || {}
...
function renderArray (tokens) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    const token = tokens[i]; const f = renderer[token.type]
...
if (noStarch) {
  const epiStart = tokens.findIndex(t => t.type == 'meta_quote_open' && t.args[0]?.chapter)
  if (epiStart > -1) {
    const epiEnd = tokens.findIndex((t, i) => t.type == 'meta_quote_close' && i > epiStart)
    const range = tokens.slice(epiStart, epiEnd + 1)
    tokens.splice(epiStart, epiEnd + 1 - epiStart)
    tokens.splice(0, 0, ...range)
    const image = tokens.findIndex(t => t.type == 'meta_figure' && t.args[0]?.chapter)
    if (image) {
