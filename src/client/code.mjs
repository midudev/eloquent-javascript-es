...
const modeGuesser = EditorState.transactionExtender.of(tr => {
  ...
  if (tr.docChanged && tr.startState.facet(contextFacet).type == null &&
      (guessed = guessType(tr.newDoc)) != tr.startState.facet(language).name) { return { effects: updateLanguage(guessed) } }
  return null
})
...
class CodeSandbox {
  constructor () {
    ...
    document.querySelector('#solution').addEventListener('click', () => {
      const context = this.editor.state.facet(contextFacet)
  ...
  selectContext (value) {
    this.output.clear()
    this.clearSandbox()
    const chapter = getChapter(this.chapters.value); let visible
    if (value == 'box') {
      ...
      const guessed = guessType(chapter.start_code)
      if (guessed == 'javascript') { code = '// ' + code } else { code = '<!-- ' + code + ' -->' }
      if (chapter.start_code) code += '\n\n' + chapter.start_code
      this.setEditorState(code, { include: chapter.include })
      visible = 'box'
    } else {
      const exercise = findExercise(value, chapter)
      if (exercise.goto) {
      ...
      if (/\.zip$/.test(exercise.file)) { link.href = '../' + exercise.file } else { link.href = 'data:text/plain;charset=UTF-8,' + encodeURIComponent(exercise.solution) }
    }
  ...
  runCode () {
    this.clearSandbox()
    const val = this.editor.state.doc.toString(); const type = this.editor.state.facet(language).name
    const context = this.editor.state.facet(contextFacet)
    Sandbox.create({
      ...
      place: type == 'html' &&
        function (node) {
          const out = document.querySelector('.sandbox-output')
          out.parentNode.insertBefore(node, out)
        }
    }).then(box => {
      this.sandbox = box
      this.output.clear()
      if (type == 'html') { box.setHTML(val, this.output) } else { box.run(val, this.output) }
    })
  }
  ...
  selectChapter (number, context) {
    ...
    if (chapter.exercises.length) {
      ...
      chapter.exercises.forEach(exercise => {
        const num = chapter.number + '.' + exercise.number
  ...
  parseFragment () {
    ...
    if (valid) {
      const perValue = exercise ? hash : 'box'; let setPer = false
...
function hasIncludes (code, include) {
  ...
  const re = /(?:\s|<!--.*?-->)*<script src="([^"]*)"><\/script>/g; let m
  const found = []
...
function getChapter (number) {
  for (let i = 0; i < chapterData.length; i++) {
    if (chapterData[i].number == number) { return chapterData[i] }
  }
}
...
function findExercise (id, chapter) {
  ...
  for (let i = 0; i < chapter.exercises.length; i++) {
    if (chapter.exercises[i].number == parts[1]) { return chapter.exercises[i] }
  }
}

if (window.page && window.page.type == 'code') { window.codeSandbox = new CodeSandbox() }
