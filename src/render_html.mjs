...
function highlight (lang, text) {
  let result = ''; const parser = parsers[lang]; const tree = parser ? parser.parse(text) : Tree.empty
...
const renderer = {
  fence (token) {
    const config = /\S/.test(token.info) ? PJSON.parse(token.info) : {}
    if (config.hidden) return ''
    const lang = config.lang || 'javascript'; const tab = lang == 'html' || lang == 'javascript' ? ' tabindex="0"' : ''
  ...
  meta_figure (token) {
    const { url, alt, chapter } = token.args[0]
    const className = !chapter ? null : 'chapter' + (chapter == 'true' ? '' : ' ' + chapter)
...
const template = mold.bake('chapter', fs.readFileSync(dir + `/${epub ? 'epub_' : ''}chapter.html`, 'utf8'))
